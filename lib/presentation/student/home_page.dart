...
import '../../infrastructure/notice/notice_service.dart';
import '../../widgets/chat_list_item.dart';
import 'chat_page.dart';
import 'login_page.dart';
import '../../infrastructure/school_data.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _fetchNotices() async {
    ...
    final result = await NoticeService.getNotices();
      ...
      (error) {
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Failed to load notices: ${error.toString()}')),
          );
        }
      },
      (response) {
  ...
  void _navigateToPrincipalNotice() {
    // Find the Principal Notice chat from the chats list
    final principalNoticeChat = chats.firstWhere(
      (chat) => chat.name == 'Principal Notice',
      orElse: () => chats.first, // Fallback to first chat if not found
    );

    // Navigate to the Principal Notice chat
    navigateToChat(principalNoticeChat);
  }
  ...
  void navigateToChat(Chat chat) {
    // If it's Principal Notice, navigate to Notice Board instead
    if (chat.name == 'Principal Notice') {
      Navigator.pushNamed(
        context,
        Routes.notice,
        arguments: notices,
              ...
              children: [
                Row(
                  children: [
                    ...
                    // Notice Icon - Navigate to Principal Notice
                    Container(
                      decoration: BoxDecoration(
                      ...
                      child: Stack(
                        children: [
                          IconButton(
                            ...
                            onPressed: _navigateToPrincipalNotice,
                            tooltip: 'Principal Notice',
                          ),
